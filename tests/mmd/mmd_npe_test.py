...
if __name__ == "__main__":
    ...
    fig, ax = plot_coverage(
        [alpha, alpha_2],
        [ecp, ecp_2],
        labels=[f'NM {npe_train_sim}-{npe_train_sim}', f'NM {npe_train_sim}-{dann_sim}'],
    )
    plt.savefig(f'mmd_rank_m{dann_sim}_{npe_train_sim}_{test_obs}.png', bbox_inches='tight')
    plt.clf()
    ...
    ax[0].legend([f'N {npe_train_sim.upper()}-{npe_train_sim.upper()}',
                   ...
                   f'NM {npe_train_sim.upper()}-{npe_train_sim.upper()}',
                   f'NM {npe_train_sim.upper()}-{dann_sim.upper()}'],)
    ...
    plt.savefig(f'mmd_true_pred_m{dann_sim}_{npe_train_sim}_{test_obs}_halo.png', bbox_inches='tight')
    plt.clf()
